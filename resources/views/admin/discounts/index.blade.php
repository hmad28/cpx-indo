        ...
        <div class="md:max-w-7xl 2xl:max-w-full mx-auto sm:px-6 lg:px-8 2xl:px-15">
            <div class="bg-white overflow-hidden rounded-xl border border-gray-200 shadow-sm">
                <div class="text-gray-900 p-5">
                    <div class="p-6">
                        <h1 class="text-2xl font-black text-gray-950 mb-4">Kelola Diskon</h1>
                        ...
                        {{-- List Diskon --}}
                        <h2 class="text-xl font-bold text-gray-950 mb-3">Daftar Diskon</h2>
                            ...
                            <table class="w-full border text-sm sm:text-base">
                                <thead>
                                    <tr class="bg-gray-950 text-white">
                                        ...
                                        <th class="p-3 whitespace-nowrap text-left font-semibold">Persentase Diskon</th>
                                        <th class="p-3 whitespace-nowrap text-left font-semibold">Tanggal Mulai</th>
                                        <th class="p-3 whitespace-nowrap text-left font-semibold">Tanggal Akhir</th>
                                ...
                                <tbody>
                                    @forelse($diskons as $diskon)
                                        <tr class="hover:bg-gray-50 border-b border-gray-100">
                                            <td class="p-3">{{ $diskon->product->name }}</td>
                                            <td class="p-3">{{ $diskon->discount_percentage }}%</td>
                                            <td class="p-3">{{ $diskon->start_date }}</td>
                                            <td class="p-3">{{ $diskon->end_date }}</td>
                                            <td class="p-3">
                                                <span class="px-2 py-1 rounded text-xs font-semibold {{ $diskon->status == 'active' ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800' }}">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="text-center p-6 text-gray-500">Belum ada diskon</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                        ...
                        <div class="md:hidden space-y-4">
                            @forelse($diskons as $diskon)
                                <div class="border border-gray-200 rounded-xl p-4 shadow-sm flex flex-col gap-2">
                                    <div class="text-base font-semibold text-gray-950">{{ $diskon->product->name }}</div>
