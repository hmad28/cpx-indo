...
INSERT INTO testimonials (name, position, message)
SELECT seed.name, seed.position, seed.message
FROM (VALUES
...
) AS seed(name, position, message)
WHERE NOT EXISTS (
    SELECT 1 FROM testimonials testimonial WHERE testimonial.name = seed.name
);
